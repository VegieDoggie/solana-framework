...
import {TransferTransactionData, TokenTransferParsed, TokenAccountParsed, TransferParsed} from "../types/transfer.ts";
...
export async function mustGetParsedTransactions(connection: Connection, signatures: string[], retrySec: number = 0.5) {
    while (true) {
        try {
...
export async function getSignatures(connection: Connection, target: PublicKey, start: string, includeErrTx?: boolean) {
    const limit = 1000;

    async function _getSignatures(end?: string): Promise<ConfirmedSignatureInfo[]> {
        ...
        if (signatures.length === limit) {
            return [...signatures, ...await _getSignatures(signatures.at(-1)!.signature)];
        }
...
export async function sendInstructions(
    ...
    return sendTransaction(connection, recentSignedTransaction, maxExecErrorRetry, commitment)
}
...
export async function sendTransaction(connection: Connection, recentSignedTransaction: () => Promise<Transaction | VersionedTransaction>, maxExecErrorRetry: number = 1, commitment?: 'processed' | 'confirmed' | 'finalized') {
...
export async function isExistAccount(connection: Connection, address: PublicKey, commitmentOrConfig?: Commitment | GetAccountInfoConfig) {
    while (true) {
        try {
...
export async function parseTransactions(connection: Connection, signaturesInfos: ConfirmedSignatureInfo[]) {
        ...
        for (let j = 0; j < instructions.length; j++) {
            ...
            if (instruction.parsed?.type === "transfer") {
                switch (instruction.program) {
                    case "spl-token": {
                        const data = await parseTokenTransferData(connection, instruction, signature, j, transaction);
                        if (data) list.push(data);
                        break
                    }
                    case "system": {
                        const data = parseTransferData(instruction, signature, j, transaction);
                        if (data) list.push(data);
                        break
                    }
                    default:
                        console.log("Unsupported-Transfer", signature, instruction.program, instruction.parsed);
                }
            }
        }
    }
...
async function parseTokenTransferData(connection: Connection, instruction: ParsedInstruction, signature: string, index: number, tx: ParsedTransactionWithMeta) {
    const {info} = instruction.parsed as TokenTransferParsed;
    if (Number(info.amount) > 0) {
        const publicKeys = [new PublicKey(info.source), new PublicKey(info.destination)];
        ...
        const data: TransferTransactionData = {
            ...
            processed_at: tx.blockTime ? new Date(tx.blockTime).toString() : undefined
        }
        return data;
    }
}

function parseTransferData(instruction: ParsedInstruction, signature: string, index: number, tx: ParsedTransactionWithMeta) {
